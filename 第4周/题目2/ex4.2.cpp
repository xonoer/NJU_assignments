...
bool DDZPlayer::legal(vector<Card> cards) {
    ...
    if (prePos == position || prePos == -1){
    	...
    	CardType ct=judgeType(current);
    	if(t!=ct&&ct!=boom&&ct!=maxboom) return false;
    	if(t==sig||t==doub||t==trip){
    		if(ct==boom||ct==maxboom) return true;
    		int c,l;
    		for(int i=0;i<15;i++) {
    			if(current[i]) c=i;
    			if(lastCard[i]) l=i;
    	...
    	if(t==con1||t==con2||t==plane){
    		...
    		if(c_s>l_s&&(c_f-c_s)==(l_f-l_s)) return true;
    		else return false;
    	}
    	if(t==p1||t==p2){
    		if(ct==boom||ct==maxboom) return true;
